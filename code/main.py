...
def initialize_model(session, model, train_dir, expect_exists):
    ...
    if ckpt and (tf.gfile.Exists(ckpt.model_checkpoint_path) or tf.gfile.Exists(v2_path)):
        ...
        if expect_exists:
            ...
            ## unable to query cuda rnn num of variables
            print 'Num params: %d' % sum(map(lambda x: x.get_shape().num_elements() if x.get_shape().num_elements() is not None else 0, tf.trainable_variables()))
...
def main(unused_argv):
    ...
    if FLAGS.mode == "train":
        ...
        with open(os.path.join(FLAGS.train_dir, "flags.json"), 'w') as fout:
            ...
            json.dump(tf.app.flags.FLAGS.flag_values_dict(), fout)
            #print FLAGS.__flags
