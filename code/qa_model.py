...
## BiDAFAttn
from modules_bidaf import BiDAFAttn
## CoAttn
from modules_coattention import CoAttn
## SelfAttn
from modules_rnnattention import RNNAttn
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        if re.search(r'lstm',self.FLAGS.experiment_name):
            encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob, cell_type='lstm')
        else:
        ...
        if self.FLAGS.experiment_name in ("baseline", "baseline_lstm"):
            attn_layer = BasicAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
        elif self.FLAGS.experiment_name in ("bidaf_lstm"):
            attn_layer = BiDAFAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
        elif self.FLAGS.experiment_name in ("coattention_lstm"):
            attn_layer = CoAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
        elif self.FLAGS.experiment_name in ("rnnattention_lstm"):
            attn_layer = RNNAttn(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
        ...
        if self.FLAGS.experiment_name not in ("rnnattention_lstm"):
            # Concat attn_output to context_hiddens to get blended_reps
            blended_reps = tf.concat([context_hiddens, attn_output], axis=2) # (batch_size, context_len, hidden_size*4)
        else:
            ## rnnattention already do this inside the attention module
        ...
        with vs.variable_scope("StartDist"):
            #if self.FLAGS.experiment_name == "baseline" or self.FLAGS.experiment_name == "baseline_lstm":
